package com.capstone.Model.JqGrid;
...
import com.capstone.Model.ObjectMapper.CustomObjectMapper;
/**
...
public class JqGridData {
	...
	public JqGridData(Map<String, Object> paramMap, List<Map<String, Object>> rows) {
		...
		if (records < pageRow) {
			this.rows = rows;
		} else if ((page - 1) * pageRow < records) {
		...
		/*
		 * if(pageRow * pageRow + pageRow < records){ this.rows = rows.subList(
		 * (page-1) * pageRow, page * pageRow + pageRow); }
		 * 
		 * else{ if(records == 1){ this.rows = rows.subList( (page-1)* pageRow,
		 * records); }else{ this.rows = rows.subList( (page-1)* pageRow, records
		 * -1);
		 * 
		 * } }
		 */
