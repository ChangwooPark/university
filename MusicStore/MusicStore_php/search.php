...
<body>
	...
	$cate = $search_cate;
	if ($cate == "1") {
		$q = "select * from music where artist like '%".$search."%' order by idx desc";
	}
	else {
		$q = "select * from music where music like '%".$search."%'";
	}
	$result = $mysqli->query($q) or die(__FILE__.":Line".__LINE__."<p>Query:$result<br><br><br>".$mysqli->error);
